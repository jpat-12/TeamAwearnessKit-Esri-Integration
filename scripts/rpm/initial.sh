...
# Setup CSV-XML.py
echo "We will now set up the CSV-XML.py script for converting CSV data into XML."
## Copy the file from the github repo
cp /tmp/TeamAwearnessKit-Esri-Integration/python-files/csv-xml.py /opt/TAK-Esri
## Test The File
python3 /opt/TAK-Esri/csv-xml.py
## Double check the output
cat /var/www/html/survey-xml.xml
echo "Does survey-xml.xml have contents (y/n)"
read csvxml
if [ "$csvxml" != "y" ]; then
    echo "CSV-XML HAS FAILED" >> /opt/TAK-Esri/install-log.txt
    echo "You may need to contact the repo manager to gain assistance with this error"
    exit 1
else
    echo "while True:" >> /opt/TAK-Esri/csv-xml.py
    echo "    parse_csv_and_create_xml('survey.csv', '/var/www/html/survey-xml.xml')" >> /opt/TAK-Esri/csv-xml.py
    echo "    print('Parsed CSV and updated survey-xml.xml')" >> /opt/TAK-Esri/csv-xml.py
    echo "    time.sleep(5)" >> /opt/TAK-Esri/csv-xml.py
    clear
    echo "The output of XML messages is now in the file /var/www/html/survey-xml.xml"
fi

# Configure httpd for file serving
echo "Configuring HTTP server to serve the XML and TXT files."
...
echo "You should now be able to access your files via the HTTP server and see updates in the XML and TXT files."

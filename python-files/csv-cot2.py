...
def create_cot_event(data):
    ...
    event = (
         f'<event version="2.0" uid="Survey123_{data["objectid"]}" type="a-h-G" time="{event_time_str}" '
         f'start="{event_time_str}" stale="{stale_time_str}" how="m-g">'
         f'<point lat="{lat}" lon="{lon}" hae="0" ce="10.0" le="2.0" />'
         f'<detail><UID>Survey123_{data["team_callsign"]} '
         f'{datetime.fromtimestamp(int(data["CreationDate"]) / 1000, timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]}</UID>'

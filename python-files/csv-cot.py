...
def create_cot_event(data):
    ...
    event = f'<event version="2.0" uid="_{data["objectid"]}" time="{event_time}" start="{event_time}" stale="{datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3]}Z"><point lat="{lat}" lon="{lon}" hae="0" ce="10.0" le="2.0" /><detail><UID Droid="" /><usericon iconsetpath="{icon_path}" /><remarks>{remarks}</remarks><contact callsign="{data["team_callsign"]}" /><track speed="0" course="0" /></detail></event>'
    return event
